...
export default function WeatherInfo() {
    const { viewingData, location, viewingHourOffset } = useWeatherStore();

    // Use dynamic viewingData for proper Time Travel binding
    const displayWeather = viewingData.weather;

    // Use dynamic date if available (Time Travel), otherwise current local time
    const formattedDate = viewingData.dateStr
        ? getFormattedDate(viewingData.dateStr)
        : getFormattedDate(location.localtime);

    // Details from dynamic data
    const { feelsLike, humidity, windKph } = displayWeather;
        ...
        <>
            ...
            <div className="absolute top-10 right-6 z-50 flex flex-col gap-2 animate-fadeIn" style={{ animationDelay: '0.2s' }}>
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/20 backdrop-blur-md border border-white/10 shadow-sm ml-auto">
                    ...
                    <span className="text-white text-xs font-semibold">{feelsLike}°</span>
                </div>
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/20 backdrop-blur-md border border-white/10 shadow-sm ml-auto">
                    ...
                    <span className="text-white text-xs font-semibold">{Math.round(windKph)} <span className="text-[9px] font-normal opacity-70">km/h</span></span>
                </div>
            </div>
        </>
    );
